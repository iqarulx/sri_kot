...
class BillingOneEditInv extends StatefulWidget {
  final InvoiceModel? invoice;
  const BillingOneEditInv({super.key, this.invoice});

  @override
  State<BillingOneEditInv> createState() => _BillingOneEditInvState();
}

class _BillingOneEditInvState extends State<BillingOneEditInv>
  ...
  CartDrawerInv cartDrawer() {
    return CartDrawerInv(
      isEdit: true,
      pageType: 1,
      docId: widget.invoice!.docID ?? '',
      isConnected: isConnected,
      billNo: widget.invoice!.billNo != null ? widget.invoice!.billNo! : '',
    );
  }
    ...
    if (findCartIndex != -1) {
      ...
      if (value != null && value != "0" && value.isNotEmpty) {
        setState(() {
          ...
          billPageProviderInv.toggletab(true);
        });
      } else {
        setState(() {
          //  product qrt
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (connectionProvider.isConnected) {
        ...
        billingHandler = getProductList();
        billPageProviderInv.addListener(pageReference);
        setState(() {
          isConnected = true;
        });
      } else {
  ...
  Future getProductList() async {
    try {
      ...
      if (cid != null) {
        var category = await storeProvider.categoryListing(cid: cid);
        var product = await storeProvider.productListing(cid: cid);
        if (category != null &&
            category.docs.isNotEmpty &&
            product != null &&
            product.docs.isNotEmpty) {
          for (var category in category.docs) {
          ...
          for (var product in product.docs) {
          ...
          await LocalDB.setInvoiceParty(partyInfo.toMap());
          return true;
        }
      } else {
        setState(() {
