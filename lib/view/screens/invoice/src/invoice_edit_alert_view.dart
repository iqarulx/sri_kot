...
class InvoiceEditAlertView extends StatefulWidget {
  ...
  const InvoiceEditAlertView(
      {super.key, required this.productDataList, required this.editProduct});
...
class _InvoiceEditAlertViewState extends State<InvoiceEditAlertView> {
  ...
  showProductAlert() async {
    await showDialog(
      context: context,
      builder: (context) =>
          ProductListingDialog(productDataList: widget.productDataList),
    ).then((value) {
      if (value != null) {
  ...
  initFn() {
    setState(() {
      ...
      rate.text =
          double.parse(widget.editProduct.rate.toString()).toStringAsFixed(0);
  ...
  updateProduct() async {
    if (productFormKey.currentState!.validate()) {
      setState(() {
        ...
        widget.editProduct.total =
            double.parse(qty.text) * double.parse(rate.text);
