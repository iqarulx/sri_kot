...
class AccessCode extends FirebaseQuery {
  ...
  static Future<Map<String, dynamic>> createAccessCode() async {
    try {
      ...
      DocumentReference docRef = await FirebaseQuery.accessCode.add({
        'code': code,
      ...
      LogConfig.addLog("${DateTime.now()} : $e");
      rethrow;
    }
  }
  ...
  static Future expireCode({required String code}) async {
    try {
      var result =
          await FirebaseQuery.accessCode.where('code', isEqualTo: code).get();
      if (result.docs.isNotEmpty) {
        for (var data in result.docs) {
          await FirebaseQuery.accessCode
              .doc(data.id)
