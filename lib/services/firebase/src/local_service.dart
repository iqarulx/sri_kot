...
class LocalService {
  static Future<bool> checkFileAvailable() async {
    try {
      ...
      LogConfig.addLog(
          "${DateTime.now()} : Error checking file availability: $e");
  ...
  static Future<Map<String, List<Map<String, String>>>> getFiles() async {
    ...
    try {
      ...
      LogConfig.addLog("${DateTime.now()} : Error fetching files: $e");
    }
  ...
  static Future<Map<String, dynamic>> getHSN() async {
    ...
    var result = await FirebaseQuery.profile.doc(uid).get();
    if (result.exists) {
      return result["hsn"];
    }
    return {};
  ...
  static Future updateHSN(String hsnValue, String taxValue,
      {required bool value}) async {
    var uid = await LocalDB.fetchInfo(type: LocalData.companyid) ?? '';
    if (value) {
      await FirebaseQuery.profile.doc(uid).update({
        "hsn.common_hsn": value,
        "hsn.common_hsn_value": hsnValue,
      }).then((company) async {
        await FirebaseQuery.product
            .where("company_id", isEqualTo: uid)
            .get()
            .then((product) async {
          if (product.docs.isNotEmpty) {
            for (var data in product.docs) {
              await FirebaseQuery.product.doc(data.id).update({
                "hsn_code": hsnValue,
                "tax_value": taxValue,
              });
            }
        ...
        await FirebaseQuery.category
            .where("company_id", isEqualTo: uid)
            .get()
            .then((category) async {
          if (category.docs.isNotEmpty) {
            for (var data in category.docs) {
              await FirebaseQuery.category.doc(data.id).update({
                "hsn_code": hsnValue,
                "tax_value": taxValue,
              });
            }
  ...
  static updateLogin({required String uid}) async {
    var result = await FirebaseQuery.getId(
        collection: FirebaseQuery.profile, docId: uid);
    ...
    if (result != null && result.exists) {
      if (result["free_trial"]["opened"] == null) {
        await FirebaseQuery.update(
          collection: FirebaseQuery.profile,
          data: {
            "free_trial": {
              "opened": DateTime.now(),
              "ends_in": DateTime.now().add(const Duration(days: 3))
            }
          },
          docId: uid,
        );
  ...
  static Future<int> _getCount(String collection, String companyId) async {
    try {
      ...
      LogConfig.addLog(
          "${DateTime.now()} : Error getting count for $collection: $e");
      return 0;
    }
  }
  ...
  static Future<bool> updateCount({
    ...
    try {
      ...
      if (!result.exists) {
        ...
        print(previousCount);
        await FirebaseQuery.profile.doc(result.id).update({
  ...
  //   await FirebaseQuery.customer.add(data).then((value) {
  //     print("Data added");
  //   });
  // }
  ...
  static Future updateEnquiry({
    ...
    try {
      ...
      LogConfig.addLog("${DateTime.now()} : Error updating enquiry: $e");
  ...
  static Future updateEstimate({
    ...
    try {
      ...
      LogConfig.addLog("${DateTime.now()} : Error updating estimate: $e");
      rethrow;
    }
  }
    ...
    try {
      ...
      if (localEnquiry.isNotEmpty) {
        for (var data in localEnquiry) {
          ...
          await FirebaseQuery.insert(
                  collection: FirebaseQuery.enquiry, data: map)
              .then((value) async {
            if (value != null) {
              ...
              await FirebaseQuery.update(
                collection: FirebaseQuery.enquiry,
                docId: value,
                data: {
                  "enquiry_id":
      ...
      if (localEstimate.isNotEmpty) {
        for (var data in localEstimate) {
          ...
          await FirebaseQuery.insert(
                  collection: FirebaseQuery.estimate, data: map)
              .then((value) async {
            if (value != null) {
              final productsJson = data['products'] as String;
              final List<dynamic> productsList = jsonDecode(productsJson);
              ...
              await FirebaseQuery.update(
                collection: FirebaseQuery.estimate,
                docId: value,
                data: {
                  "estimate_id":
  ...
  static Future<AggregateQuerySnapshot?> getLastIdEnquiry() async {
    ...
    try {
      resultData = await FirebaseQuery.enquiry
          .where('company_id',
              isEqualTo:
                  await LocalDB.fetchInfo(type: LocalData.companyid) ?? '')
  ...
  static Future<AggregateQuerySnapshot?> getLastIdEstimate() async {
    AggregateQuerySnapshot? resultData;
    try {
      resultData = await FirebaseQuery.estimate
          .where('company_id',
              isEqualTo:
                  await LocalDB.fetchInfo(type: LocalData.companyid) ?? '')
      ...
      await dbHelper.dropTable('product').then((value) async {
        await dbHelper.checkAndCreateTable('product').then((value) async {
          for (var value in productData) {
            ...
            await dbHelper.insertProduct(data: data);
          }
        });
      });
    } catch (e) {
        ...
        await dbHelper.checkAndCreateTable('category').then((value) async {
          if (categoryData.isNotEmpty) {
            for (var value in categoryData) {
              ...
              await dbHelper.insertCategory(data: data);
            }
          }
        });
      });
        ...
        await dbHelper.checkAndCreateTable('customer').then((value) async {
          if (customerData.isNotEmpty) {
            for (var value in customerData) {
              ...
              await dbHelper.insertCustomer(data: data);
            }
          }
        });
      });
  ...
  static Future<List<QuerySnapshot>> getDeletedItems() async {
    ...
    try {
      var staff = await FirebaseQuery.staff
          .where('company_id', isEqualTo: uid)
          .where('deleted_at', isEqualTo: false)
          .get();
      result.add(staff);
      var product = await FirebaseQuery.product
          .where('company_id', isEqualTo: uid)
          .where('deleted_at', isEqualTo: false)
          .get();
      result.add(product);
      var enquiry = await FirebaseQuery.enquiry
          .where('company_id', isEqualTo: uid)
          .where('deleted_at', isEqualTo: false)
          .get();
      result.add(enquiry);
      var estimate = await FirebaseQuery.estimate
          .where('company_id', isEqualTo: uid)
          .where('deleted_at', isEqualTo: false)
          .get();
      result.add(estimate);
  ...
  static Future<QuerySnapshot<Map<String, dynamic>>> getAppversion() async {
    try {
      QuerySnapshot<Map<String, dynamic>> snapshot = await FirebaseQuery
          .appVersion
  ...
  static Future updateExpiryDate() async {
    try {
      ...
      await FirebaseQuery.profile.doc(companyId).update({
  ...
  static Future addStaffCount() async {
    try {
      var companyId = await LocalDB.fetchInfo(type: LocalData.companyid);
      var result = await FirebaseQuery.profile.doc(companyId).get();
  ...
  static Future addBulkStaffCount() async {
    try {
      var companyId = await LocalDB.fetchInfo(type: LocalData.companyid);
      var result = await FirebaseQuery.profile.doc(companyId).get();
      var previousCount = result["max_staff_count"];
      await FirebaseQuery.profile.doc(companyId).update({
        "max_staff_count": previousCount + 10,
      });
    } on Exception catch (e) {
      LogConfig.addLog("${DateTime.now()} : ${e.toString()}");
    }
  ...
  static Future<QuerySnapshot<Map<String, dynamic>>>
    ...
    try {
      var companyId = await LocalDB.fetchInfo(type: LocalData.companyid);
      var result = await FirebaseQuery.purchases
          .where('company_id', isEqualTo: companyId)
          .get();
      return result;
    } on Exception catch (e) {
