...
class EnquiryExcel {
  ...
  Future<List<int>?> createCustomerExcel() async {
    ...
    if (isEstimate) {
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: 0)).value = TextCellValue('Estimate ID');
    } else {
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: 0)).value = TextCellValue('Enquiry ID');
    }
    ...
    sheet.cell(CellIndex.indexByColumnRow(columnIndex: 4, rowIndex: 0)).value = TextCellValue('Enquiry Date');
    ...
    for (var i = 0; i < enquiryData.length; i++) {
      ...
      if (isEstimate) {
        sheet.cell(CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: (i + 1))).value =
            TextCellValue(enquiryData[i].estimateid ?? enquiryData[i].referenceId ?? '');
      } else {
        sheet.cell(CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: (i + 1))).value =
            TextCellValue(enquiryData[i].enquiryid ?? enquiryData[i].referenceId ?? '');
      }
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 2, rowIndex: (i + 1))).value = TextCellValue(
          enquiryData[i].customer != null && enquiryData[i].customer!.customerName != null
              ? enquiryData[i].customer!.customerName!
              : "");
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 3, rowIndex: (i + 1))).value = TextCellValue(
          enquiryData[i].customer != null && enquiryData[i].customer!.address != null
              ? enquiryData[i].customer!.address!
              : "");
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 4, rowIndex: (i + 1))).value =
          TextCellValue(DateFormat('dd-MM-yyyy hh:mm a').format(enquiryData[i].createddate!));
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 5, rowIndex: (i + 1))).value =
          TextCellValue(enquiryData[i].price!.total!.toStringAsFixed(2));
    }
